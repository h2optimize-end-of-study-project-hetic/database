...
MQTT_TOPIC = os.getenv("MQTT_TOPIC", "$SYS/#")
...
dbname = os.getenv("POSTGRES_RECORD_DB", "recorded")
user = os.getenv("POSTGRES_USER", "hoptimize")
password = os.getenv("POSTGRES_PASSWORD", "Changeme!1")
...
def connect_pg():
    for i in range(5):
        try:
            ...
            print(f"[PG] PostgreSQL indisponible (tentative {i+1}/10) : {e}")
            time.sleep(10)
    print("[PG] Erreur critique : impossible de se connecter à PostgreSQL.")
    exit(1)
...
def validate_field(data, key, expected_type):
    """Valide un champ et retourne (valeur ou None, score)"""
    try:
        value = data.get(key)
...
def insert_data(data):
    ...
    try:
        ts_raw, s_ts = validate_field(data, "tx_time_ms_epoch", (int, float))
        ts = to_ts(ts_raw) if ts_raw else datetime.now(timezone.utc)
    except Exception as e:
        print(f"[ERROR] Insertion échouée : erreur timestamp : {e}")
        traceback.print_exc()
        return
    
    if not isinstance(data, dict):
        print(f"[WARN] insert_data ignoré (non dict): {type(data).__name__} -> {data}")
        return
    ...
    print(f"[INSERT] sensor_id={sid}, event_id={eid}, time={ts}, data={d}")
    ...
    try:
        if sid == 112:
            ...
            if valid_count > 0:
                print(f"[INFO] {valid_count} voisins insérés (moyenne pertinence = {relevance_total/valid_count:.2f})")
        elif sid == 116:
            ...
            print(f"[WARN] Sensor ID inconnu : {sid}")
        conn.commit()
...
def on_subscribe(client, userdata, mid, reason_code_list, properties):
    if reason_code_list[0].is_failure:
        ...
        print(f"Broker granted QoS: {reason_code_list[0].value}")

def on_unsubscribe(client, userdata, mid, reason_code_list, properties):
    print("Unsubscribe succeeded" if len(reason_code_list) == 0 or not reason_code_list[0].is_failure
          else f"Broker replied with failure: {reason_code_list[0]}")
    print("Still running. Waiting for manual stop...")
...
def on_message(client, userdata, message):
    ...
    try:
        ...
        if not isinstance(data, dict):
            print(f"[WARN] Message ignoré (non dict): {type(data).__name__} -> {data}")
            return
        insert_data(data)
    except Exception as e:
        print(f"[ERROR] Erreur de traitement du message : {e}")
        traceback.print_exc()
...
def on_connect(client, userdata, flags, reason_code, properties):
    if reason_code.is_failure:
        ...
        print(f"Connected to broker. Subscribing to topics: {userdata['topics']}")
        client.subscribe([(topic, 0) for topic in userdata["topics"]])
